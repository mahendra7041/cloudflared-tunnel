  ...
  tunnel(retries = 0) {
      ...
      write(chunk, _, callback) {
        ...
        if (connected) {
          if (url) {
            ...
            self.currentStatus = {
              status: "error",
              message: "Could not start Cloudflare tunnel: URL not found.",
            };
          }
        }
    ...
    exec(getBinPathTarget(), args, {
      ...
      externalErrorHandler: async (error) => {
        ...
        outputDebug(
          `Cloudflare tunnel crashed: ${error.message}, restarting...`
        );
...
function findError(data) {
  ...
  return `Could not start Cloudflare tunnel: ${cleanCloudflareLog(
    data.toString()
  )}`;
}
...
function findConnection(data) {
  const match =
    data
      .toString()
      .match(/(INF Registered tunnel connection|INF Connection)/) ?? undefined;
