...
export default async function install(
  ...
  if (env.SHOPIFY_CLI_IGNORE_CLOUDFLARED) {
    outputDebug(
      "Skipping cloudflared installation because SHOPIFY_CLI_IGNORE_CLOUDFLARED is set"
    );
    return;
  }
  ...
  if (fileExistsSync(binTarget)) {
    ...
    try {
      const versionArray = execFileSync(binTarget, ["--version"], {
        encoding: "utf8",
      }).split(" ");
      const versionNumber = versionArray.length > 2 ? versionArray[2] : "0.0.0";
      const needsUpdate = versionIsGreaterThan(
        CURRENT_CLOUDFLARE_VERSION,
        versionNumber
      );
      if (!needsUpdate) {
...
async function downloadFile(url, to) {
  ...
  if (!response.ok || !response.body)
    throw new Error(
      `Couldn't download file ${url} (${response.status} ${response.statusText})`
    );
